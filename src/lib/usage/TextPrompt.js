      ...
      try {
         ...
         if (this._currentUsage.possibles.length === 1) {
            return this.handleError(
               error ||
                  (this.args[this.params.length] === undefined ? this.message.language.get('COMMANDMESSAGE_MISSING_REQUIRED', possible.name) : err)
            );
         }
         return this.handleError(
            error ||
               this.message.language.get(
                  'COMMANDMESSAGE_NOMATCH',
                  this.message.guild.settings.get('prefix'),
                  this.message.command,
                  this._currentUsage.possibles.map((poss) => poss.name)
               )
         );
      }
   }
