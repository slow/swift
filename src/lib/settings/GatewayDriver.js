...
class GatewayDriver {
   ...
   register(name, { provider = this.client.options.providers.default, schema = new Schema() } = {}) {
      ...
      if (this.name !== undefined && this.name !== null)
         throw new Error(`The key '${name}' is either taken by another Gateway or reserved for GatewayDriver's functionality.`);
   ...
   sync(input) {
      return Promise.all(
         [...this].map(([key, gateway]) => gateway.sync(typeof input === 'undefined' ? this.client.options.gateways[key].syncArg : input))
      );
   }
   ...
   *[Symbol.iterator]() {
   ...
   toJSON() {
      return {
         ...
         ...Object.assign(
            {},
            [...this].map(([key, value]) => ({ [key]: value.toJSON() }))
         )
      };
   }
